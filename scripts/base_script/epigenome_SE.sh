...
if [ "$fasterq" = true ]; then
	...
	. ~/script/rna-pipe/fasterq_SE.sh 2> Fasterq.${err_log2} 1> Fasterq.${oup_log2}

	finalize_process "fasterq-dump" $start "${oup_log}" "${err_log}" "./${R}.${fq_ext}.gz"
	cd ../
...
if [ "$fastp" = true ]; then
	...
	finalize_process "Fastp" $start "${oup_log}" "${err_log}" "./${R}.fastp.${fq_ext}"

	if [ "$rm_fasterq" = true ]; then
		rm ${inp_dir}/${R}.${fq_ext}.gz
	fi
...
if [ "$bowtie2" = true ]; then
	...
	samtools view \
		-@ ${thread} \
		-Sb ${R}.sam -o ${R}.pre.bam \
		>> BT2.${oup_log2} 2>&1  &&
		
	rm ${R}.sam &&

	samtools sort \
		-@ ${thread} -o ${R}.pre.sort.bam ${R}.pre.bam \
		>> BT2.${oup_log2} 2>&1  &&

	rm ${R}.pre.bam &&

	picard -Xmx24g MarkDuplicates \
		--INPUT ${R}.pre.sort.bam \
		--OUTPUT ${R}.pre.sort.markdup.bam \
		--ASSUME_SORTED true \
	...
	samtools view \
		...
		-F 0x0400 \
		-q 1 \
		-b ${R}.pre.sort.markdup.bam \
		1> ${R}.pre.sort.markdup.filt.bam \
		2>> BT2.${oup_log2}  &&

	rm ${R}.pre.sort.markdup.bam &&

	bedtools intersect \
		-a ${R}.pre.sort.markdup.filt.bam \
		-b ${exclude} \
		-v \
		-ubam \
		1> ${R}.bam \
	...
	rm ${R}.pre.sort.markdup.filt.bam &&
	rm -r ${sample}.tmp &&
	
	samtools index ${R}.bam &&

	finalize_process "bowtie2" $start "${oup_log}" "${err_log}" "${R}.bam.bai"
	
	if [ "$rm_fastp" = true ]; then
                rm ../01_fastp/${R}.fastp.${fq_ext}
...
if "${bam2cov}" ; then
	...
	finalize_process "BamCoverage" $start "${oup_log}" "${err_log}" "./${R}.bw"
